...
/*mat4 Projection, ModelView, NormalMatrix;
vec4 vPosition;
vec4 normal;
vec3 R;
*/
...
function initWindowEvents() {
    ...
    canvas.onmousemove = function(e) {
		...
		lightPosition[0] = -radius * Math.cos(theta) * Math.cos(phi);
		lightPosition[1] = -radius * Math.sin(phi);
		lightPosition[2] = radius * Math.sin(theta) * Math.cos(phi);
...
function buildSurfaceOfRevolution(controlPoints, angles, steps)
{
    ...
    for (var i = 0; i < 2; i++)
    {
		...
		var t3 = subtract(ee, dd);
		var t4 = subtract(ff, ee);
		normal = cross(t3, t4);
		normal = vec3(normal);
		normalsArray.push(normal);
		normalsArray.push(normal);
...
function updateLightView(light) {
    gl.uniformMatrix4fv(light, false, flatten(light));
}
...
function updateMaterial(material) {
    if(material == "brass") {
        ...
        lightAmbient = vec4(.2, 0.2, 0.2, 1.0 );
        lightDiffuse = vec4( 0.5, 0.5, 0.5, 0.5 );
        lightSpecular = vec4( 1.0, 1.0, 1.0, 1.0 );

        materialAmbient = vec4( 1.0, 0.0, 1.0, 1.0 );
        materialDiffuse = vec4( 0.2, 0.2, 0.2, 1.0);
        materialSpecular = vec4( 1.0, .5, 0.0, 0.0 );
        materialShininess = 50.0;
	gl.uniform3fv(locations.triangleColor, vec3(1,.5,0));
      ...
      materialAmbient = vec4( 0.5, 0.5, 0.0, 1.0 );
...
window.onload = function() {
    ...
    var img = document.getElementById("tile-img");
